...
export default function TeacherAssignments() {
    ...
    const fetchSubmissions = async (assignmentId) => {
        try {
            const res = await fetch(`http://localhost:5000/api/assignment/${assignmentId}/submissions`);
            const data = await res.json();
            if (res.ok) {
                setSubmissions(data);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                fetchAssignments();
                setForm({ title: "", description: "", maxMarks: "", questionFile: null });
    ...
    const handleDeleteAssignment = async (assignmentId) => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/api/assignment/${assignmentId}`, {
                method: "DELETE",
            });
        ...
        <div className="min-h-screen bg-gray-100 py-10 px-5 flex flex-col items-center">
            <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-8 rounded-xl shadow">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="border border-dashed p-4 rounded bg-gray-50 flex flex-col">
                            <label className="text-sm font-semibold mb-2">Upload Question File (Image Only)</label>
                            <input
                                type="file"
                                accept="image/*"
                ...
                <div className="bg-white p-8 rounded-xl shadow flex flex-col">
                    ...
                    <div className="overflow-y-auto max-h-[500px] flex-grow space-y-4">
                        {assignments.map((a) => (
                            <div key={a._id} className="border p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition flex justify-between items-center group">
                                <div className="cursor-pointer flex-grow" onClick={() => fetchSubmissions(a._id)}>
                                    <h3 className="font-bold text-lg text-blue-700">{a.title}</h3>
                                    <p className="text-sm text-gray-500">Max Marks: {a.maxMarks}</p>
                                </div>
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        onClick={(e) => { e.stopPropagation(); handleDeleteAssignment(a._id); }}
            ...
            {selectedAssignmentId && (
                <div className="w-full max-w-4xl mt-8 bg-white p-8 rounded-xl shadow">
                    <h2 className="text-2xl font-bold mb-6 text-gray-800">Student Submissions (AI Evaluated)</h2>
                    {submissions.length === 0 ? (
                        <p className="text-gray-500 italic">No submissions yet for this assignment.</p>
                    ) : (
                        <div className="space-y-6">
                            {submissions.map((sub) => (
                                <div key={sub._id} className="border p-4 rounded-lg flex flex-col md:flex-row gap-6">
                                    {/* Left: Image */}
                                    <div className="md:w-1/3">
                                        <p className="text-sm font-semibold text-gray-600 mb-1">Student Answer</p>
                                        <a href={`http://localhost:5000${sub.answerFilePath}`} target="_blank" rel="noreferrer">
                                            <img
                                                src={`http://localhost:5000${sub.answerFilePath}`}
                                                alt="Submission"
                                    ...
                                    <div className="md:w-2/3 flex flex-col justify-center">
                                        <h3 className="font-bold text-xl mb-1">{sub.studentId?.name || "Unknown Student"} <span className="text-sm text-gray-500">({sub.studentId?.usn || "N/A"})</span></h3>

                                        {sub.status === "failed_evaluation" ? (
                                            <div className="bg-red-50 text-red-700 p-3 rounded mt-2 border border-red-200">
                                                <span className="font-bold">⚠️ AI Evaluation Failed.</span> Manual grading required.
                                            </div>
                                        ) : (
                                            <>

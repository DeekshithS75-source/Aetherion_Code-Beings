...
export default function StudentAssignments() {
    ...
    const fetchAssignments = async () => {
        try {
            const res = await fetch("http://localhost:5000/api/assignment/all");
    ...
    const fetchMySubmissions = async () => {
        try {
            const res = await fetch(`http://localhost:5000/api/assignment/student/${studentId}`);
            const data = await res.json();
            if (res.ok) {
                const subMap = {};
                data.forEach(sub => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch("http://localhost:5000/api/assignment/submit", {
                method: "POST",
                body: formData,
            });
            ...
            <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            return (
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        {isCompleted ? (
                                            <span className="bg-green-100 text-green-800 text-xs font-bold px-2 py-1 rounded">Submitted</span>
                                        ) : (
                                    ...
                                    {isCompleted && submission.status === "graded" && (
                                        <div className="mt-3 bg-gray-100 p-2 rounded text-sm flex gap-2 items-center font-semibold">
                                            <span>Score: {submission.score} / {a.maxMarks}</span>
                                        </div>
                                    )}
                                </div>
                            );
                ...
                <div className="bg-white p-8 rounded-xl shadow h-fit">
                    {!selectedAssignment ? (
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-blue-700 mb-2">{selectedAssignment.title}</h2>
                            <p className="text-sm font-semibold text-gray-500 mb-4">Posted by: {selectedAssignment.teacherId?.name} • Max Marks: {selectedAssignment.maxMarks}</p>
                            ...
                            <div className="mb-6">
                                <p className="text-sm font-bold text-gray-600 mb-2">Reference Image:</p>
                                <a href={`http://localhost:5000${selectedAssignment.questionFilePath}`} target="_blank" rel="noreferrer">
                                    <img
                                        src={`http://localhost:5000${selectedAssignment.questionFilePath}`}
                                        alt="Question"
                            ...
                            {mySubmissions[selectedAssignment._id] ? (
                                <div className="border border-green-200 bg-green-50 p-6 rounded-lg text-center">
                                    ...
                                    <h3 className="text-lg font-bold text-green-700 mb-1">Assignment Submitted</h3>
                                    {mySubmissions[selectedAssignment._id].status === "graded" && (
                                        <div className="mt-4 text-left">
                                            <p className="font-bold text-lg mb-2">Grade: {mySubmissions[selectedAssignment._id].score} / {selectedAssignment.maxMarks}</p>
                                            <div className="bg-white p-3 rounded border text-sm text-gray-700">
                                                <span className="font-bold block mb-1">AI Teacher Feedback:</span>
                                ...
                                <form onSubmit={handleSubmit} className="border border-blue-200 bg-blue-50 p-6 rounded-lg text-center">
                                    <h3 className="font-bold text-blue-800 mb-4">Submit Your Answer</h3>
                                    <p className="text-sm text-gray-600 mb-4">Please upload a clear, handwritten image of your answer.</p>

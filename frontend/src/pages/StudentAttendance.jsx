...
export default function StudentAttendance() {
    ...
    const fetchAttendance = async (usnToFetch) => {
        setLoading(true);
        try {
            const res = await fetch(`http://localhost:5000/api/attendance/student/${usnToFetch}`);
        ...
        <div className="min-h-screen bg-gray-100 py-10 px-5 flex flex-col items-center">
            ...
            {!hasUsn ? (
                <div className="bg-white p-8 rounded-xl shadow w-full max-w-md">
                    <h2 className="text-2xl font-bold mb-6 text-center text-blue-600">My Attendance</h2>
                    <p className="text-gray-600 mb-6 text-center text-sm">Please enter your USN to view your attendance history.</p>
                ...
                <div className="w-full max-w-4xl bg-white p-8 rounded-xl shadow">
                    <div className="flex justify-between items-center mb-6 border-b pb-4">
                        <div>
                            <h2 className="text-3xl font-bold text-gray-800">My Attendance Log</h2>
                            <p className="font-mono text-gray-500 mt-1">Showing records for: <span className="font-bold text-blue-600">{usnInput}</span></p>
                        </div>
                        <button onClick={() => setHasUsn(false)} className="bg-gray-200 text-gray-700 px-4 py-2 rounded font-semibold hover:bg-gray-300 transition">Change USN</button>
                    </div>

                    {loading ? (
                        <p className="text-gray-500 text-center py-10">Loading records...</p>
                    ) : records.length === 0 ? (
                        <p className="text-gray-500 text-center py-10">No attendance records found for this USN.</p>
                    ) : (
